...
if : #Summation option
    ...
    num = int(input("What positive number between 0 and 10000 do you want to operate?\n"))
    
    if :         #Numbers have to be between 0 and 10000 to be valid
        
        res = 0
        for in:  #How many operations are we going to make?
            
            res =         #What do we need to add?
...
elif :   #Factorial option

    num = int(input("What positive number do you want to operate?\n"))
    
    if :         #Negative number (less than zero) is invalid, numbers bigger than 50 will cause an error
        
        print("Invalid input")

    elif :      #Factorial of zero is just 1, fill the condition
        ...
        while : #We will multiply from num down to 1, so what do we need to check here?                                  
            res =     #What do we need to multiply?
            i =       #Decrease the conditional variable
